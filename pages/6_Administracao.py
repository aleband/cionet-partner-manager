...
sb = restore_client()
if sb is None:
    st.error("Sua sessão expirou. Volte à página principal e faça login novamente.")
    st.stop()
...
with tab_partners:
    ...
    if partners.empty:
        ...
        with st.form("edit_partner_form"):
            ...
            save_partner = st.form_submit_button("Salvar alterações", type="primary")
            if save_partner:
                if not new_name.strip():
                    ...
                    try:
                        sb.table("partners").update({
                            ...
                            "contract_model": new_model,
                            "council_exception": new_council,
                            "notes": new_notes,
        ...
        st.warning("Ao excluir um Business Partner, os registros de utilização vinculados a ele também serão removidos do banco.")
        if not can_delete:
            ...
            if st.button("Excluir Business Partner", type="secondary", disabled=not confirm_partner):
                try:
...
with tab_events:
    ...
    with st.form("new_event_form", clear_on_submit=True):
        ...
        add_event = st.form_submit_button("Adicionar evento", type="primary")
        if add_event:
    ...
    if events.empty:
        ...
        with st.form("edit_event_form"):
            ...
            save_event = st.form_submit_button("Salvar alterações", type="primary")
            if save_event:
                if not new_event_name.strip():
                    ...
                    try:
                        sb.table("events").update({
                            "name": new_event_name.strip(),
                            "event_type": new_event_type,
                            "event_date": new_event_date.isoformat(),
                            "location": new_location,
                        }).eq("id", int(ev["id"])).execute()
        ...
        st.warning("Ao excluir um evento, registros de utilização vinculados ficam sem referência ao evento, conforme a regra do banco.")
        if not can_delete:
            st.info("Somente usuários Admin podem excluir registros.")
        else:
            confirm_event = st.checkbox(f"Confirmo a exclusão de {ev['name']} em {fmt_date(ev['event_date'])}", key="confirm_delete_event")
            if st.button("Excluir evento", type="secondary", disabled=not confirm_event):
                try:
